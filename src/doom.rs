...
pub fn consume_images(
    ...
    grab_write_into("sprites", "w / 2", "h - 15", wad)?;
    grab_write_into("crouch_sprites", "w / 2", "h - 15", wad)?;
    grab_write_into("mugshots", "w / 2 - 18", "h / 2 - 17", wad)?;

    Ok(())
}

fn grab_write_into(subpath: &str, x: &str, y: &str, wad: &mut Wad) -> anyhow::Result<()> {
